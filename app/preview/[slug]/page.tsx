...
export default async function PreviewPage(props: PreviewPageProps) {
  ...
  // Fetch post regardless of published status
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {/* Preview Banner */}
      <div className="sticky top-0 z-[100] bg-amber-500 text-amber-950 text-center py-2 text-sm font-bold flex items-center justify-center gap-4">
        <span>
          PREVIEW MODE — {post.published ? "Published" : "Draft"}
        </span>
        <Link
          href={`/admin/write/${post.slug}`}
          className="inline-flex items-center gap-1 rounded-md bg-amber-600/30 px-2.5 py-1 text-xs font-bold hover:bg-amber-600/50 transition-colors"
        >
      ...
      {/* Header */}
      <header className="sticky top-[36px] z-50 w-full border-b border-slate-200/50 bg-white/80 backdrop-blur-md dark:border-slate-800/50 dark:bg-slate-950/80">
        <div className="container mx-auto flex h-16 items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="sm" href="/admin" className="font-bold text-slate-500 hover:text-slate-900">
              <ChevronLeft className="mr-1 h-4 w-4" /> Back to Admin
            </Button>
            <ThemeToggle />
          </div>
        </div>
      ...
      <footer className="border-t border-slate-200 bg-white/50 dark:border-slate-800 dark:bg-slate-950/50 py-8">
        <div className="container mx-auto px-6 text-center text-xs font-bold text-slate-400 uppercase tracking-widest" suppressHydrationWarning>
          © {new Date().getFullYear()} Preview Mode. Not publicly visible.
        </div>
      </footer>
    </div>
  );
